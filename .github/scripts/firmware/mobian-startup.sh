...
FAILSAFE_AP_CON=${FAILSAFE_AP_CON:-"openstick-ap"}
FAILSAFE_AP_SSID=${FAILSAFE_AP_SSID:-"openstick"}
FAILSAFE_AP_PASSWORD=${FAILSAFE_AP_PASSWORD:-"12345678"}
FAILSAFE_AP_CHANNEL=${FAILSAFE_AP_CHANNEL:-"3"}
FAILSAFE_AP_ADDRESS=${FAILSAFE_AP_ADDRESS:-"192.168.100.1/24"}

USB_NET_CON=${USB_NET_CON:-"usb"}
USB_NET_ADDRESS=${USB_NET_ADDRESS:-"192.168.100.1/24"}
...
USB_REGISTER_DEBUG=$USB_DEBUG/sys/kernel/debug/usb/ci_hdrc.0/registers
...
add_usb_net() {
  nmcli con del $USB_NET_CON
  nmcli con add \
    type ethernet ifname usb0 con-name "$USB_NET_CON" \
    ipv4.addresses "$USB_NET_ADDRESS" \
...
setup_gadget_rndis_network() {
  ...
  if nmcli connection up $USB_NET_CON ; then
...
diagnose_network() {
  ...
  # oops, all failed
  # activating ap interfaces
  logger "Device offline and cannot open RNDIS interface, activating AP interfaces"
